...
/// 개체 상세 — 캘린더 탭.
/// 월별 캘린더에 급여/체중/메모 기록 마커를 표시하고,
/// 날짜 탭 시 해당 날짜의 기록 목록을 바텀시트로 보여준다.
      ...
      builder: (_) => Consumer(
        builder: (context, ref, __) {
          final dayAsync = ref.watch(
              petDayTimelineProvider(PetDateParam(widget.petId, dateStr)));
          return SafeArea(
            child: Padding(
                ...
                children: [
                  ...
                  dayAsync.when(
                    ...
                    error: (_, __) => const Padding(
                      padding: EdgeInsets.symmetric(vertical: 30),
                      child: Center(child: Text('기록을 불러올 수 없어요')),
                    ),
                    data: (items) {
                      final filtered = items
                          .where((r) =>
                              _calendarCats.contains(r.category))
                          .toList();
                      if (filtered.isEmpty) {
...
/// 바텀시트 기록 행 — 개체 상세 기록 탭의 타임라인 행과 동일한 스타일
