...
- ⚠️ **`R__02` 는 파일에 없는 공식 모프를 매 실행마다 지운다** (개체가 물고 있으면 FK RESTRICT 라
  지우지 않고 **사용자 정의 모프로 전환** — `created_by` = 그 개체의 OWNER).
...
- **딥링크 공개 경로** (`public-paths`): `/.well-known/**`, `/t/**`
  ...
  - `TagLandingController` — 미설치·비로그인용 랜딩. 모르는 코드도 404 대신 일반 랜딩
    - 노출: 이름·종·모프·성별·대표사진 + **주인 닉네임** + **마지막 기록의 종류·시점**(급여 · 2일 전)
    - ⛔ **기록의 '내용'은 절대 넣지 말 것** — 체중 값, 급여 품목, 메모 본문. 종류와 시점까지가 한계다.
      코드만 알면 로그인 없이 열리는 공개 URL 이라 "돌봐지고 있다"는 사실까지만 보여준다
    - 주인 닉네임은 가계도와 **같은 규칙**(`show_nickname_in_pedigree`) — false 면 '비공개'.
      거기서 숨긴 사람이 이름표에서 드러나면 설정의 의미가 없다. 소유자 판정은 `pet_keeper_rls`(OWNER)
    - 합사·산란은 제외 — 번식 활동은 공개 이름표에 띄울 성질이 아니다
    - **부모는 이름만**(`TagLandingParent`). 가계도 부모 등록은 상대 승인이 없는 구조라 부모 칸엔
      **남의 개체가 들어올 수 있고**, 그 주인은 이 이름표에 동의한 적이 없다.
      모프는 ① 부모가 공개 개체(`private_yn='N'`) 이거나 ② 부모의 주인이 이 개체의 주인과 같을 때만 붙인다.
      ⛔ 부모의 사진·성별·주인·일련번호는 넣지 말 것. 고아 개체는 이름을 남긴다(익명화 보존의 목적이 혈통 식별)
    - 설치 유도는 **맨 아래 한 줄뿐**. 버튼으로 키우지 말 것 (광고로 보이는 순간 이름표의 신뢰가 깎인다)
